    ...
    <div className="flex flex-col lg:flex-row">
      <div className="flex-grow mr-10">
        <div className="flex mb-8 [&>h3]:w-1/3 [&>h3]:p-2 space-x-3 [&>h3]:border-b-4 [&>h3:not(:first-child)]:border-neutral-500">
          <h3 className="border-sky-500">Items in cart</h3>
          <h3>Payment</h3>
          <h3>Order received</h3>
        </div>
        <h1 className="text-4xl font-bold mb-5">
          Koszyk{" "}
          <span className="text-xl text-neutral-500 ml-1">
            ({cart.items.length})
          </span>
        </h1>
        {cart.itemsDetails.map((storeitem, i) => (
          <Fragment key={storeitem.id}>
            <CartCard
              {...storeitem}
              inCartQuantity={cart.quantityInCart(storeitem.id)}
            />
            <HorizontalSeparator />
          </Fragment>
        ))}
      ...
      <aside>
        <div className="sticky top-28 mt-12 bg-neutral-800 p-10 rounded-xl space-y-5">
          ...
          <Button className="w-full bg-blue-500 font-bold text-xl">Pay</Button>
        </div>
      </aside>
    </div>
  );
